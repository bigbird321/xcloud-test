...
-- Basic Function Begin --
  function array2string(tdlist, pos1, pos2)
    local res ="";
    for i=pos1,pos2 do
  ...
  function table2string(meta)
      local res=""
      local i
      for i=1,#meta do
          res=res..string.char(meta[i])
      end
	  res = string.tohex(res)
      return res
  end
  ...
  function getByte(str,pos)
    local byte = string.sub(str, (pos*2)+1, ((pos+1)*2))
    return byte
  end
-- Basic Function End --
...
function toPrivate(str)
    ...
    elseif json['cmd'] == 'query' then
        metaData = {0xAA, 0xBB, 0x00, 0x00, 0x00, 0x00, 0x00, 0xA0, 0xAA, 0xBB}
    else
        metaData = {0xAA, 0xBB, 0x00, 0x00, 0x00, 0x00, 0x00, 0xA0, 0xAA, 0xBB}
	end
	local meta = table2string(metaData)
    return meta
end


-- Binary To JSON
function fromPrivate(data)
  local as = {}
  -- TODO
  as['1'] = tonumber(getByte(data, 2), 16)
  as['2'] = tonumber(getByte(data, 3), 16)
  as['3'] = tonumber(getByte(data, 4), 16)
  as['4'] = tonumber(getByte(data, 5), 16)
  as['5'] = tonumber(getByte(data, 6), 16)
  as['6'] = tonumber(getByte(data, 7), 16)
...
--[[
string.sub('aaaa', 2, 2)
local testas = '{"cmd":"opt","mid":"12345","as":{"1":0,"2":1,"3":2,"4":2,"5":3,"6":100}}'
--local testas = '{"cmd":"query","mid":"12345678901234567","to":"gome10000000000018"}'

local datastr = toPrivate(testas)
print('datastr='..datastr)

local newas = fromPrivate(datastr)
print('newas='..newas)
--]]
---End: test---
